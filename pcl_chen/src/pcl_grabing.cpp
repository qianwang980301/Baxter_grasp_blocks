...
Pcl_grabing::Pcl_grabing(ros::NodeHandle* nodehandle) : CwruPclUtils(nodehandle), display_ptr_(new PointCloud<pcl::PointXYZ>) {
    TableHeight = roughHeight;
    TableColor(0) = roughColor_R;
    TableColor(1) = roughColor_G;
    TableColor(2) = roughColor_B;
    table_publisher = nodehandle->advertise<sensor_msgs::PointCloud2>("table_points", 1, true);
...
void Pcl_grabing::update_kinect_points() {
    ...
    A_sensor_wrt_torso = transformTFToEigen(tf_sensor_frame_to_torso_frame);

    save_kinect_clr_snapshot();
    while (pcl::io::loadPCDFile<pcl::PointXYZRGB> ("kinect_clr_snapshot.pcd", *pclKinect_clr_ptr_) == -1) //* load the file
    {
...
bool Pcl_grabing::findTableTop() {
    ...
    display_ptr_->width = transformed_pclKinect_clr_ptr_->width;
    ...
    pcl::toROSMsg(*display_ptr_, pcl2_display_cloud);
    pcl2_display_cloud.header.stamp = ros::Time::now();
    table_publisher.publish(pcl2_display_cloud);
    
    TableCentroid = compute_centroid(display_ptr_);
...
bool Pcl_grabing::isBlock()
{
    ...
    BlockCentroid = compute_centroid(display_ptr_);
    ...
    block_ptr_->width=display_ptr_->width;
    block_ptr_->height=display_ptr_->height;
    block_ptr_->points.resize(n_block_top);   
    for (int i = 0; i < n_block_top; i++) 
    {
        block_ptr_->points[i].getVector3fMap()=display_ptr_->points[index[i]].getVector3fMap();
    }
    
    
    BlockTopCentroid = compute_centroid(block_ptr_);
    ...
    double block_dist;
    fit_points_to_plane(block_ptr_,Block_Normal,block_dist);
    Block_Major = get_major_axis();
